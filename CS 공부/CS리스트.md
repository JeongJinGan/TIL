...
- javascript를 쉽게 활용할 수 있도록 도와주는 오픈소스 기반의 Javascript 라비르러리
- 특징
  - 웹 페이지 상에서 element를 쉽게 찾고 조작 가능
  - 거의 모든 웹 브라우저에 대응할 정도로 호환성이 매우 뛰어남
...
- 하지만 HTML처럼 데이터를 보여주는 목적이 아닌, 데이터를 저장하고 전달할 목적



### 트러블 슈팅

- 무언가 문제가 발생했을때 원인을 찾아 제거
- 왜 필요할까??
  - 초기 대응을 잘못하면 문제가 더 커지기 쉽다
  - 서버나 시스템에 장애가 일어나면 그 기간동안 업무가 정지



### API

- 응용 프로그램에서 사용할 수 있도록, 운영 체제 혹은 프로그래밍 언어가 제공하는 기능을 제어할 수있게 만든 인터페이스
  - 인터페이스란? 
...
- 자원을 이름으로 구분하여 해당 자원의 상태를 주고 받는 모든 것
  - 1. HTTP URI를 통해 자원을 명시
    2. HTTP Method(get, post, delete 등)를 통해
    3. 해당자원(URI)에 대한 CRUD Operation을 적용하는 것
